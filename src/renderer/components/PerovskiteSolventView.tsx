...
import SolventRoleBadge from './SolventRoleBadge';
import SortTableHeader from './SortTableHeader';
import BookmarkButton from './BookmarkButton';
import { usePerovskiteSolvent } from '../hooks/usePerovskiteSolvent';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'role';

export default function PerovskiteSolventView() {
  const [deltaD, setDeltaD] = useState(18.5);
  const [deltaP, setDeltaP] = useState(12.0);
  const [deltaH, setDeltaH] = useState(9.5);
  const [r0, setR0] = useState(7.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = usePerovskiteSolvent();
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">ペロブスカイト溶媒設計</h2>
        <p className="text-xs text-gray-500 mb-4">
          前駆体HSPに対する溶媒のREDから、良溶媒（処理溶媒）とアンチソルベントを識別します。
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">前駆体 dD</label><input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">前駆体 dP</label><input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">前駆体 dH</label><input type="number" step="0.1" value={deltaH} onChange={(e) => setDeltaH(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate} className="px-6 py-2.5 bg-blue-600 text-white rounded-md font-medium text-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{loading ? '評価中...' : '溶媒スクリーニング'}</button>
          <BookmarkButton pipeline="perovskiteSolvent" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50"><tr>
                ...
                <SortTableHeader label="役割" field="role" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
              </tr></thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5"><SolventRoleBadge role={r.role ?? 'Intermediate'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
