...
import ElectrolyteSuitabilityBadge from './ElectrolyteSuitabilityBadge';
import SortTableHeader from './SortTableHeader';
import BookmarkButton from './BookmarkButton';
import { useLiBatteryElectrolyte } from '../hooks/useLiBatteryElectrolyte';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'dielectric' | 'suitability';

export default function LiBatteryElectrolyteView() {
  const [deltaD, setDeltaD] = useState(15.0);
  const [deltaP, setDeltaP] = useState(18.0);
  const [deltaH, setDeltaH] = useState(10.0);
  const [r0, setR0] = useState(10.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useLiBatteryElectrolyte();
  ...
  const sortedResults = useMemo(() => {
    ...
    items.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        ...
        case 'dielectric': cmp = (a.estimatedDielectric ?? 0) - (b.estimatedDielectric ?? 0); break;
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">LiB電解液設計</h2>
        <p className="text-xs text-gray-500 mb-4">
          電解質塩のHSPに基づき、溶媒候補のRED値と推定誘電率から電解液適性を評価します。
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">塩 dD</label>
            <input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">塩 dP</label>
            <input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">塩 dH</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate}
            ...
            {loading ? '評価中...' : 'LiB電解液評価'}
          </button>
          <BookmarkButton pipeline="liBatteryElectrolyte" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">LiB電解液スクリーニング結果</h3>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <SortTableHeader label="推定誘電率" field="dielectric" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                  <SortTableHeader label="適性" field="suitability" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5 text-sm text-gray-500">{r.estimatedDielectric?.toFixed(1) ?? '-'}</td>
                    <td className="px-3 py-2.5"><ElectrolyteSuitabilityBadge level={r.suitability ?? 'Poor'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
