...
import { useMXeneDispersion } from '../hooks/useMXeneDispersion';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'dispersibility';

export default function MXeneDispersionView() {
  const [deltaD, setDeltaD] = useState(16.4);
  const [deltaP, setDeltaP] = useState(13.5);
  const [deltaH, setDeltaH] = useState(15.2);
  const [r0, setR0] = useState(10.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useMXeneDispersion();
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">MXene分散最適化</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、MXene(Ti3C2Tx)の分散に適した溶媒を選定します。
          MXeneは親水性表面を持つため、水やDMSO系溶媒が有効です。デフォルト値はTi3C2Tx。
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">MXene dD</label>
            <input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">MXene dP</label>
            <input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">MXene dH</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate}
            ...
            {loading ? 'スクリーニング中...' : 'MXene分散スクリーニング'}
          </button>
          <BookmarkButton pipeline="mxeneDispersion" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">MXene分散スクリーニング結果</h3>
          </div>
