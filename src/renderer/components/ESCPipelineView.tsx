...
import ESCRiskBadge from './ESCRiskBadge';
import SortTableHeader from './SortTableHeader';
import BookmarkButton from './BookmarkButton';
import { useESCPipeline } from '../hooks/useESCPipeline';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'riskLevel';

export default function ESCPipelineView() {
  const [selectedGroup, setSelectedGroup] = useState<PartsGroup | null>(null);

  const { result, loading, error, evaluate, clear } = useESCPipeline();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">環境応力亀裂(ESC)リスク評価</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、ポリマー部品と溶媒の組み合わせによる環境応力亀裂(ESC)リスクを評価します。
          RED値が低いほどESCリスクが高くなります。
        </p>
        ...
        <div className="flex gap-3">
          ...
          >
            {loading ? '評価中...' : 'ESCリスク評価'}
          </button>
          <BookmarkButton
            pipeline="escPipeline"
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">
              ESCリスク評価結果: {result.partsGroup?.name ?? selectedGroup?.name}
            </h3>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <SortTableHeader label="ESCリスク" field="riskLevel" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                </tr>
              </thead>
                ...
                {sortedResults.map((r: any, i: number) => (
                  <tr key={r.solvent?.id ?? i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5">
                      <ESCRiskBadge level={r.riskLevel ?? 'Safe'} />
                    </td>
                  </tr>
                ))}
              </tbody>
