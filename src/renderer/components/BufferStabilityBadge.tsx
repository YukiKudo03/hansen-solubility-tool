...
interface BufferStabilityBadgeProps {
  level: string;
}
...
const BADGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  Marginal: { label: '境界的', bg: 'bg-yellow-100', text: 'text-yellow-800' },
  Poor: { label: '不良', bg: 'bg-red-100', text: 'text-red-800' },
};

export default function BufferStabilityBadge({ level }: BufferStabilityBadgeProps) {
