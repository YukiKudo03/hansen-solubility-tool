...
import { useCleaningFormulation } from '../hooks/useCleaningFormulation';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'cleaningLevel';

export default function CleaningFormulationView() {
  const [deltaD, setDeltaD] = useState(17.5);
  const [deltaP, setDeltaP] = useState(1.0);
  const [deltaH, setDeltaH] = useState(2.0);
  const [r0, setR0] = useState(6.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useCleaningFormulation();
  ...
  const sortedResults = useMemo(() => {
    ...
    items.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        ...
        case 'cleaningLevel': cmp = (a.cleaningLevel ?? '').localeCompare(b.cleaningLevel ?? ''); break;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">洗浄剤配合設計</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、汚れ（soil）に対する洗浄溶媒の効果を評価します。
          RED値が低いほど洗浄効果が高いことを示します。
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">汚れ dD</label>
            <input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">汚れ dP</label>
            <input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">汚れ dH</label>
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">洗浄溶媒候補（溶媒DB）</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate}
            ...
            {loading ? '評価中...' : '洗浄剤配合設計評価'}
          </button>
          <BookmarkButton pipeline="cleaningFormulation" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">洗浄剤配合設計結果</h3>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <SortTableHeader label="洗浄効果" field="cleaningLevel" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5"><CleaningLevelBadge level={r.cleaningLevel ?? 'Poor'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
