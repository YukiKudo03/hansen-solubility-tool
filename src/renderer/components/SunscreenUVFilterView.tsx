...
import UVFilterSolubilityBadge from './UVFilterSolubilityBadge';
import SortTableHeader from './SortTableHeader';
import BookmarkButton from './BookmarkButton';
import { useSunscreenUVFilter } from '../hooks/useSunscreenUVFilter';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'uvFilterName' | 'ra' | 'red' | 'solubility';

export default function SunscreenUVFilterView() {
  const [deltaD, setDeltaD] = useState(16.0);
  const [deltaP, setDeltaP] = useState(3.0);
  const [deltaH, setDeltaH] = useState(5.0);
  const [r0, setR0] = useState(8.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useSunscreenUVFilter();
  ...
  const sortedResults = useMemo(() => {
    ...
    items.sort((a: any, b: any) => {
      let cmp = 0;
      switch (sortKey) {
        case 'uvFilterName': cmp = (a.uvFilter?.name ?? '').localeCompare(b.uvFilter?.name ?? '', 'ja'); break;
        case 'ra': cmp = (a.ra ?? 0) - (b.ra ?? 0); break;
        case 'red': cmp = (a.red ?? 0) - (b.red ?? 0); break;
        case 'solubility': cmp = (a.solubility ?? '').localeCompare(b.solubility ?? ''); break;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">UVフィルター適合性評価</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、ビヒクル（基剤）中でのUVフィルターの溶解性を評価します。
          RED値が低いほどSPF確保に有利です。RED &ge; 1.0では結晶析出リスクがあります。
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ビヒクル dD</label>
            <input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ビヒクル dP</label>
            <input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ビヒクル dH</label>
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">UVフィルター（溶媒DB）</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate}
            ...
            {loading ? '評価中...' : 'UVフィルター適合性評価'}
          </button>
          <BookmarkButton pipeline="sunscreenUVFilter" params={{ deltaD, deltaP, deltaH, r0, uvFilterIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">UVフィルター適合性結果</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <SortTableHeader label="UVフィルター名" field="uvFilterName" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                  ...
                  <SortTableHeader label="溶解性" field="solubility" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-3 py-2.5 text-sm font-medium text-gray-900">{r.uvFilter?.name ?? '-'}</td>
                    ...
                    <td className="px-3 py-2.5"><UVFilterSolubilityBadge level={r.solubility ?? 'Poor'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
