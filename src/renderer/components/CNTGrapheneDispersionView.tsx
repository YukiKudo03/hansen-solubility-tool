...
import { useCNTGrapheneDispersion } from '../hooks/useCNTGrapheneDispersion';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'dispersibility';

export default function CNTGrapheneDispersionView() {
  const [deltaD, setDeltaD] = useState(17.8);
  const [deltaP, setDeltaP] = useState(7.5);
  const [deltaH, setDeltaH] = useState(7.6);
  const [r0, setR0] = useState(4.5);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useCNTGrapheneDispersion();
  ...
  const sortedResults = useMemo(() => {
    ...
    items.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        ...
        case 'dispersibility': cmp = (a.dispersibility ?? 0) - (b.dispersibility ?? 0); break;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">CNT/グラフェン分散溶媒スクリーニング</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、CNT/グラフェン系ナノ材料の分散に適した溶媒を選定します。
          RED値が低いほど良好な分散が期待できます。デフォルト値はSWCNT。
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ナノ材料 dD</label>
            <input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ナノ材料 dP</label>
            <input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ナノ材料 dH</label>
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">溶媒選択</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate}
            ...
            {loading ? 'スクリーニング中...' : 'CNT/グラフェン分散スクリーニング'}
          </button>
          <BookmarkButton pipeline="cntGrapheneDispersion" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-800">CNT/グラフェン分散スクリーニング結果</h3>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5"><DispersibilityBadge level={r.dispersibility ?? 5} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
