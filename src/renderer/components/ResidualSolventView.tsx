...
import ResidualLevelBadge from './ResidualLevelBadge';
import SortTableHeader from './SortTableHeader';
import BookmarkButton from './BookmarkButton';
import { useResidualSolvent } from '../hooks/useResidualSolvent';
import { useSortableTable } from '../hooks/useSortableTable';

type SortKey = 'solventName' | 'ra' | 'red' | 'residualLevel';

export default function ResidualSolventView() {
  const [deltaD, setDeltaD] = useState(18.6);
  const [deltaP, setDeltaP] = useState(10.5);
  const [deltaH, setDeltaH] = useState(5.1);
  const [r0, setR0] = useState(8.0);
  const [solvents, setSolvents] = useState<any[]>([]);
  const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { result, loading, error, evaluate, clear } = useResidualSolvent();
  ...
  const sortedResults = useMemo(() => {
    ...
    items.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        ...
        case 'residualLevel': cmp = (a.residualLevel ?? '').localeCompare(b.residualLevel ?? ''); break;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">残留溶媒予測</h2>
        <p className="text-xs text-gray-500 mb-4">
          HSPに基づき、膜（フィルム）中に残留する溶媒リスクを予測します。
          RED値が低いほど残留リスクが高く、RED値が高いほど溶媒が膜から脱離しやすいことを示します。
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">膜 dD</label><input type="number" step="0.1" value={deltaD} onChange={(e) => setDeltaD(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">膜 dP</label><input type="number" step="0.1" value={deltaP} onChange={(e) => setDeltaP(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">膜 dH</label><input type="number" step="0.1" value={deltaH} onChange={(e) => setDeltaH(Number(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" /></div>
        ...
        <div className="flex gap-3">
          <button onClick={handleEvaluate} disabled={!canEvaluate} className="px-6 py-2.5 bg-blue-600 text-white rounded-md font-medium text-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{loading ? '評価中...' : '残留溶媒予測'}</button>
          <BookmarkButton pipeline="residualSolvent" params={{ deltaD, deltaP, deltaH, r0, solventIds: selectedIds }} disabled={selectedIds.length === 0} />
        </div>
      </div>
      ...
      {result && sortedResults.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200"><h3 className="text-sm font-semibold text-gray-800">残留溶媒予測結果</h3></div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50"><tr>
                ...
                <SortTableHeader label="残留リスク" field="residualLevel" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} />
              </tr></thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedResults.map((r: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5"><ResidualLevelBadge level={r.residualLevel ?? 'HighResidual'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
