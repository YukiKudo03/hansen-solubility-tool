...
interface DrugLoadingBadgeProps {
  level: string;
}

const BADGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  High: { label: '高', bg: 'bg-green-100', text: 'text-green-800' },
  Medium: { label: '中', bg: 'bg-yellow-100', text: 'text-yellow-800' },
  Low: { label: '低', bg: 'bg-red-100', text: 'text-red-800' },
};

export default function DrugLoadingBadge({ level }: DrugLoadingBadgeProps) {
