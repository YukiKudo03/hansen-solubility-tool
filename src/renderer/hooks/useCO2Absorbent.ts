...
export function useCO2Absorbent() {
  ...
  const evaluate = useCallback(async (params: {
    ...
    setError(null);
    try {
      const evalResult = await (window as any).api.screenCO2Absorbents(params);
      setResult(evalResult);
      return evalResult;
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'CO2吸収材評価中にエラーが発生しました';
      setError(msg);
      return null;
    } finally {
      setLoading(false);
