...
export function usePrintedElectronics() {
  ...
  const evaluate = useCallback(async (params: {
    ...
    setError(null);
    try {
      const evalResult = await (window as any).api.evaluatePrintedElectronicsWetting(params);
      setResult(evalResult);
      return evalResult;
    } catch (e) {
      const msg = e instanceof Error ? e.message : '印刷電子濡れ性評価中にエラーが発生しました';
      setError(msg);
      return null;
    } finally {
      setLoading(false);
