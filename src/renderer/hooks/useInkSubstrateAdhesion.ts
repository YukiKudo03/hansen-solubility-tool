...
export function useInkSubstrateAdhesion() {
  ...
  const evaluate = useCallback(async (params: {
    ...
    setError(null);
    try {
      const evalResult = await (window as any).api.evaluateInkSubstrateAdhesion(params);
      setResult(evalResult);
      return evalResult;
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'インク-基材密着評価中にエラーが発生しました';
      setError(msg);
      return null;
    } finally {
      setLoading(false);
