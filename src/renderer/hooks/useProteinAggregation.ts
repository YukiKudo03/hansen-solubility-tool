...
export function useProteinAggregation() {
  ...
  const evaluate = useCallback(async (params: {
    ...
    setError(null);
    try {
      const evalResult = await (window as any).api.evaluateProteinAggregation(params);
      setResult(evalResult);
      return evalResult;
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'タンパク質凝集リスク評価中にエラーが発生しました';
      setError(msg);
      return null;
    } finally {
      setLoading(false);
